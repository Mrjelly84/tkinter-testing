...
class InputForm(ttk.Frame):
    def __init__(self, parent):
        ...
        self.entry_add = ttk.Button(self, text="Add Line", command=self.add_to_list)
        self.entry_add.grid(row=0, column=1)

        self.entry_remove = ttk.Button(self, text="Remove Line", command=self.remove_to_list)
        self.entry_remove.grid(row=0, column=2)

        self.entry_clear = ttk.Button(self, text="Clear", command=self.clear_list)
        self.entry_clear.grid(row=0, column=3)

        self.entry_save = ttk.Button(self, text="Save", command=lambda: save_list(self, self.text_list))
        self.entry_save.grid(row=0, column=4)

        self.entry_load = ttk.Button(self, text="Load", command=lambda: load_list(self, self.text_list))
        self.entry_load.grid(row=0, column=5)
